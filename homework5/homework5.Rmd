...
date: "Due on March 13, 2022 at 11:59 pm"
output: pdf_document
---
...
We know
$logit\left(\theta_i\left(x_i\right)\right) = \alpha + \beta x_i$ and $logit\left(v\right) = log\left(\frac{v}{1-v}\right)$. So we can do
$$log\left(\frac{\theta_i(x_i)}{1-\theta_i(x_i)}\right) = \alpha + \beta x_i$$
$$exp(log(\frac{\theta_i(x_i)}{1-\theta_i(x_i)})) = e^{\alpha + \beta x_i}$$
...
$$ e^{\alpha + \beta x_i} = 0.5 + 0.5e^{\alpha + \beta x_i} $$ 
$$e^{\alpha + \beta x_i} - 0.5e^{\alpha + \beta x_i} = 0.5$$
$$0.5e^{\alpha + \beta x_i} = 0.5$$
$$e^{\alpha + \beta x_i} = 1$$
\center{$\alpha + \beta x_i = ln(1)$ and $ln(1) = 0$

$\alpha = -\beta x_i$ which then makes} 
$$x_i = -\frac{\alpha}{\beta}$$
...
logistic_fit <- sampling(stan_model("beehive.stan"), data = list(N = length(y), x = x, y=y), refresh = 0)
logistic_samples <- extract(logistic_fit)

alpha_samples <- logistic_samples$alpha
beta_samples <- logistic_samples$beta

ld50 <- mean(-alpha_samples/beta_samples)
print(ld50)
```
...
compute_curve <- function(sample) {
  ...
  prob <- inv_logit(alpha + beta*xgrid)
  prob
...
samples <-  mh_logistic(c(0, 0), 1000, 10000)
ld50_samples <- -samples[,"alpha"]/samples[,"beta"]

ld50_ess <- effectiveSize(ld50_samples)
...
ld50_samples_new <- -samples_new[,"alpha"]/samples_new[,"beta"]
...
rmse_complete_pooling <- sqrt(mean((phat_pooled - val)^2))
rmse_no_pooling <- sqrt(mean((phat_mle - val)^2))
...
```
_The "no pooling" estimator gives a lower RMSE, indicating that in this case, it gives a better estimated of the end of year batting averages._
...
As $\tau$ approaches $\infty$, $p{i}$ will approach the no pooling estimator.  If $\tau$ is large, then the skill differences between players is assumed to be large, meaning that each parameter must be estimated separately.  On the other hand, as $\tau$ approaches 0, $p{i}$ will approach the complete pooling estimator. If $\tau$ is 0, then all players are assumed to have the same skill level, which is the same assumption that the complete pooling estimator has.
...
rmse_partial_pooling <- sqrt(mean((pm - val)^2))
...
_The partial pooling estimator had the lowest estimator._
...
_Looking at the plot, we can see the variance of the MLEs is large, while the variance of the posterior mean estimates is much smaller, showing how the posterior means "shrink" the empirical batting averages._
