...
date: "__Due on Sunday, January 16, at 11:59 pm__"
output: pdf_document
urlcolor: blue
---
...
$$L(\lambda) = \prod_{i=1}^{n} \frac{\lambda^{y_i}e^{-\lambda}}{y_{i}!}$$
$$L(\lambda) = e^{-n\lambda}\prod_{i=1}^{n} \frac{\lambda^{y_i}}{y_{i}!}$$
$$L(\lambda) = \frac{e^{-n\lambda}\lambda^{\sum^{n}_{i=1}y_i}}{\prod^{n}_{i=1}y_i!}$$
$$L(\lambda) \propto e^{-n\lambda} \lambda^{\sum^{n}_{i=1}y_i}$$
...
$$\ell(\lambda) = ln(e^{-n\lambda} \lambda^{\sum^{n}_{i=1}y_i})$$
$$\ell(\lambda) = ln(e^{-n\lambda}) + ln(\lambda^{\sum^{n}_{i=1}y_i})$$
$$\ell(\lambda) = -n\lambda ln(e) + \sum^{n}_{i=1}y_{i}ln(\lambda)$$
$$\ell(\lambda) = \sum^{n}_{i=1}y_{i}ln(\lambda)  -n\lambda$$
Taking derivative of $\ell(\lambda)$:
$$\ell'(\lambda) = \frac{\sum^{n}_{i=1}y_{i}}{\lambda} - n$$
Set = 0
$$0 =\frac{\sum^{n}_{i=1}y_{i}}{\lambda} - n $$
$$n = \frac{\sum^{n}_{i=1}y_{i}}{\lambda}$$
$$\hat{\lambda} = \frac{\sum^{n}_{i=1}y_{i}}{n}$$
...
# YOUR CODE HERE
ggplot(data = text_tb, aes(x = chapter, y = dark_counts)) + geom_bar(stat = "identity")
```
...
p1 <- ggplot(data = data.frame(lambda = 0), mapping = aes(x=lambda)) + geom_function(fun=L1) + xlim(0,50) + scale_y_continuous(name = "Likelihood") + geom_vline(xintercept = sum(x)/n, color = 'red') 
p1
```
...
Our interpretation of the quantity $\frac{\nu_i}{1000}$ is the chapter length per 1000 words and $\lambda$ in this model is the expected count of the word dark out of 1000 words.
...
$$L\left( \lambda \cdot \frac{\nu_i}{1000} \right) = \prod_{i=1}^{n} \frac{(\lambda \cdot \frac{\nu_i}{1000})^{y_i}e^{-\lambda \cdot \frac{\nu_i}{1000}}}{y_{i}!}$$
$$L\left( \lambda \cdot \frac{\nu_i}{1000} \right) = \frac{ e^{-\lambda\sum^{n}_{i=1}\frac{\nu_i}{1000}}\left( \frac{\nu_i^{\sum{y_i}}}{1000^{\sum{y_i}}} \cdot \lambda^{\sum{y_i}}\right)}{\prod{y_i}!} $$
$$
L\left( \lambda \cdot \frac{\nu_i}{1000} \right) \propto e^{-\lambda\sum^{n}_{i=1}\frac{\nu_i}{1000}} \cdot \lambda^{\sum{y_i}}
$$
$$\ell\left(L\left( \lambda \cdot \frac{\nu_i}{1000} \right) \right) = -\lambda\sum^{n}_{i=1}\frac{\nu_i}{1000} + \sum{y_i}ln(\lambda) $$

$$\ell'\left(L\left( \lambda \cdot \frac{\nu_i}{1000} \right) \right) = -\sum^{n}_{i=1}\frac{\nu_i}{1000} + \frac{\sum{y_i}}{\lambda}$$
$$0 = -\sum^{n}_{i=1}\frac{\nu_i}{1000} + \frac{\sum{y_i}}{\lambda} $$
$$\hat\lambda = \frac{\sum{y_i}}{\sum{\frac{\nu_i}{1000}}}$$
...
```
The MLE is basically stating the estimate of the appearance of the word _dark_ every 1000 words. The maximum likelihood estimate is 0.965, which is essentially saying that for every 1000 words, we can expect on average that there is 0.965 counts of dark (1 count of dark basically if rounded up).
...
n <- 37
x <- dark_counts
L2 <- function(lambda){-lambda * (sum(chapter_lengths)/1000) + sum(x) * log(lambda)}
p2 <- ggplot(data = data.frame(lambda = 0), mapping = aes(x=lambda)) + geom_function(fun=L2) + xlim(0,10) + scale_y_continuous(name = "Likelihood") + geom_vline(xintercept = lambda_mle, color = 'red') 
p2
```
...
simulated_histogram <- ggplot(simulated_counts) + 
...
$$
Y_i \sim \hbox{Poisson} (\lambda_i \cdot \frac{\nu_i}{1000}) \quad \hbox{ for } \quad i=1,...,n.
$$
...
lambda_hats <- dark_counts / (chapter_lengths / 1000)

darkest_chapter <- which.max(lambda_hats)

# Make a bar plot of the MLEs, lambda_hats
ggplot(data = text_tb, aes(x = chapter, y = lambda_hats)) + geom_bar(stat = "identity")
```
...
darkest_chapter
```
Chapter 23 has the highest usage of the word _dark_.
...
# YOUR CODE HERE
lambda <- rgamma(1000, shape = 10, rate = 1)
...
The variance is greater than the mean, which makes sense. The reason this makes sense is because the rate of use for the word _dark_ is not the same for each chapter, so it makes sense that the variance is higher.
...
Known, Var $= 0$: $\alpha$, $\beta$, $y_1, ..., y_n$, $n$
...
# Extra Credit.
Compute $p(Y_i \mid \alpha, \beta) = \int p(Y_i, \lambda_i \mid \alpha, \beta) d\lambda_i$.  _Hint:_ Use the fact that the gamma function is defined as $\Gamma ( z ) = \int _ { 0 } ^ { \infty } x ^ { z - 1 } e ^ { - x } d x$, doont' try to do  any integrals yourself. Look at your sheet of distributions and see if you can find a matching distribution.
